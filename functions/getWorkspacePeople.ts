...
Deno.serve(async (req) => {
  try {
    ...
    for (const invite of invites) {
      const acceptedUser = allUsers.find((item) => item.email?.toLowerCase() === invite.invited_email?.toLowerCase());
      if (acceptedUser) {
        acceptedInviteEmails.add(acceptedUser.email.toLowerCase());
        if (invite.status === 'pending') {
          await base44.asServiceRole.entities.PendingInvite.update(invite.id, { status: 'accepted' });
        }
      }
    }

    const members = allUsers
      .filter((item) => item.id === organization.owner_user_id || (workspaceDomain && getEmailDomain(item.email) === workspaceDomain) || acceptedInviteEmails.has((item.email || '').toLowerCase()))
      .map((item) => ({
        id: item.id,
    ...
    const refreshedInvites = await base44.asServiceRole.entities.PendingInvite.filter({ organization_owner_user_id: organization.owner_user_id });
